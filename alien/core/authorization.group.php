...
//        return AlienContent::getInstance()->groupExists($id);
    ...
    public static function renderForm($id){
        ...
        $cp='';
        if(!Authorization::permissionTest(null,array('GROUP_VIEW'))) return;
        ...
        foreach($allPermissions as $item){
            if(in_array($item->getId(), $permissionsId)) continue;
                $cp = '<div style="float: right; display: inline-block; position: relative;">
            ...
            while($object=$STH->fetch()){
                ...
                echo ('<div class="item"><img src="images/icons/group.png"> <b>'.$group->getName().'</b>');
                    echo ('<div style="float: right; display: inline-block; position: relative;">');
                        if(Authorization::permissionTest(null, array())) echo ('<a href="'.$editAction.'"><img class="button" src="images/icons/group_edit.png" title="Edit group" alt="Edit"></a>');
                        if(Authorization::permissionTest(null, array('GROUP_EDIT'))) echo ('<a href="'.$dropAction.'"><img class="button negative" src="images/icons/cross.png" title="Delete group" alt="Delete"></a>');
